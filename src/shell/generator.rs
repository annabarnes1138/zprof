...
fn generate_zshrc(
    ...
    // Shared history configuration (must be set early to override system /etc/zshrc)
    content.push_str("# Shared history configuration\n");
...
fn generate_zshenv(profile_path: &Path) -> Result<()> {
    ...
    content.push_str("# Auto-generated by zprof from profile.toml\n");
    content.push_str("# Note: HISTFILE is set in ~/.zshenv (root) and reset in .zshrc\n");
    content.push_str("# Add custom environment variables to profile.toml instead\n\n");
...
mod tests {
    ...
    fn test_generate_zshenv() -> Result<()> {
        ...
        assert!(content.contains("Auto-generated by zprof"));
        // HISTFILE is now set in ~/.zshenv (root) and .zshrc, not in profile .zshenv
